...
module.exports = function isActive(options = {}) { // eslint-disable-line no-unused-vars
  return function (hook) {
    ...
    if(!_.get(hook, 'params.user') || _.isEmpty(hook.params.user)) {
      ...
      throw new errors.NotAuthenticated(`Cannot check if the user is active. You must not be authenticated.`);

    } else if(!_.get(hook, 'params.user.isActive')) {
